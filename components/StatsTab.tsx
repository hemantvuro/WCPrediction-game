...
export default function StatsTab({ userStats, totalFixtures }: StatsTabProps) {
  if (!userStats) {
    return (
      <div className="p-8 text-center">
        <div className="text-6xl mb-4">📊</div>
        <p className="text-gray-500">No stats available yet. Start predicting!</p>
      </div>
    );
  }

  const accuracy = totalFixtures > 0 ? ((userStats.correctPredictions || 0) / totalFixtures * 100).toFixed(0) : 0;
  const avgPoints = totalFixtures > 0 ? (userStats.totalPoints / totalFixtures).toFixed(1) : 0;
  ...
  return (
    <div className="space-y-4 pb-20">
      ...
      <div className="bg-gradient-to-br from-blue-500 to-cyan-500 rounded-2xl p-6 text-white shadow-xl">
        <h2 className="text-2xl font-bold mb-2">📈 Your Tournament Stats</h2>
        <p className="text-blue-100">Track your prediction performance</p>
      </div>
      ...
      {badges.length > 0 && (
        <div className="bg-white rounded-2xl p-6 shadow-lg">
          <h3 className="text-lg font-bold text-gray-800 mb-4">🎖️ Your Badges</h3>
